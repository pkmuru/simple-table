...
@Component({
  standalone: true,
  selector: "demo-music-followers",
  imports: [MusicTagComponent],
  template: `
    <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-start;">
      <div style="font-size:14px;" [style.color]="c.gray">{{ d.followersFormatted }}</div>
      <demo-music-tag [text]="tagText" variant="green" [theme]="theme" />
    </div>
  `,
})
export class MusicFollowersCellComponent {
  ...
  get tagText(): string {
    return `↑ +${this.d.followersGrowthFormatted} (${this.d.followersGrowthPercent.toFixed(2)}%)`;
  }
}
