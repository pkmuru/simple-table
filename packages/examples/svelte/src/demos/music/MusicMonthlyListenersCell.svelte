<script lang="ts">
  ...
  const isPos = $derived(d.monthlyListenersChange >= 0);
  const pct = $derived(Math.abs(d.monthlyListenersChangePercent).toFixed(2));
  const tagText = $derived(
    `${isPos ? "↑" : "↓"} ${isPos ? "+" : ""}${d.monthlyListenersChangeFormatted} (${pct}%)`,
  );
</script>

<div style="display:flex;flex-direction:column;gap:4px;">
  <div style="font-size:14px;color:{c.gray};">{d.monthlyListenersFormatted}</div>
