 ...
 * Same data, headers, and props as main branch.
 */
...
const currencyFormatter = ({ value }: { value?: unknown }) =>
  `$${typeof value === "number" ? (value as number).toLocaleString() : value}`;

// Define headers showcasing meaningful collapsible scenarios (same as main)
const COLLAPSIBLE_HEADERS: HeaderObject[] = [
...
export function renderCollapsibleColumnsExample(
  ...
  const { wrapper, h2 } = renderVanillaTable(COLLAPSIBLE_HEADERS, SALES_DATA, {
    ...options,
