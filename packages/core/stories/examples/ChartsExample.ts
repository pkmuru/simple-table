...
// Default args specific to ChartsExample - exported for reuse in stories and tests (match main)
export const chartsExampleDefaults = {
  columnReordering: true,
  columnResizing: true,
  selectableCells: true,
};

// Generate sample data with trends (same as main)
...
// Export data for reuse in tests (same as main CHARTS_EXAMPLE_DATA)
export const CHARTS_EXAMPLE_DATA: Row[] = [
  {
    id: 1,
    product: "Laptop Pro",
...
export function renderChartsExample(args?: Partial<UniversalVanillaArgs>): HTMLElement {
  ...
  const { wrapper, h2 } = renderVanillaTable(HEADERS, CHARTS_EXAMPLE_DATA, {
    ...options,
