...
export const generateSaaSData = (): Row[] => {
  ...
  return Array.from({ length: 200 }, () => {
    ...
    const renewalDate = `2025-${String(Math.floor(Math.random() * 12) + 1).padStart(2, "0")}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, "0")}`;
    const signUpDate = `${year}-${String(Math.floor(Math.random() * 12) + 1).padStart(2, "0")}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, "0")}`;
...
export const SAAS_HEADERS: HeaderObject[] = [
  {
    ...
    cellRenderer: ({ row, rowIndex, value }: { row: Row; rowIndex: number; value: unknown }) => {
      return `${rowIndex} ${value as string}`;
    },
  },
  {
    accessor: "segment",
